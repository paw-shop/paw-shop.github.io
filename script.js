...
function exportTerms() {
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = "alphy-terms.json";
  anchor.click();
  URL.revokeObjectURL(url);
}

function exportAnnouncementTerms() {
  const blob = new Blob([JSON.stringify(announcementTerms, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(blob);
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = "announcement-terms.json";
  anchor.click();
  URL.revokeObjectURL(url);
}
